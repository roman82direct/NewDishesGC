<x-app-layout>
    <x-slot name="header">
        ...
        @foreach(\App\Models\Maincategory::all() as $group)
            <a class="" data-bs-toggle="collapse" href="#collapse{{$group->id}}" role="button" aria-expanded="false" aria-controls="collapse{{$group->id}}">
                {{ $group->name }}
            </a>
        @endforeach
    </x-slot>
    ...
    <div class="py-8">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                @foreach(\App\Models\Maincategory::all() as $group)
                    <div class="collapse{{ ($group->id == 1) ? ' show' : '' }}" id="collapse{{$group->id}}">
